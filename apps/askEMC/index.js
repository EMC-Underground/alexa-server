...
dataTypes[2] = [] // the 3rd item in the dataTypes array is a new empty array we will now populate with SRs
insightModule2.addThisDataType(dataTypes[2], function (subDataTypesAdded) { dataTypes[2] = subDataTypesAdded; });
// Now dataTypes[2] holds an array of SRs, each one is an object such as {ItemName: "Sev 1", OpsConsoleName: "S1", suffixCode: "4"}
	...
	for (var i = 0; i < dataTypes.length; i++) { // loop through each dataType (dataType[0] is products, dataType[1] is states...)
		...
		for (var x = 0; x < dataTypes[i].length; x++) { // loop through each array of possible dataType items (ex: products)
			...
			if (dataTypes[i][x].ItemName == reqType.ItemName) { //if one of the dataType items matches what the user specified
				...
				} else if (i == 2) { // if item was a match in the 3rd array, we know it is an SR aka insightModule4 should be used
...
app.intent('OneShotGetDataIntent',
	{
		...
		"utterances":
			...
			[
				...
				"How many {-|DataType} {SRs|service requests|problems} does {Customer} have open",
				"How many {-|DataType}s {SRs|service requests|problems} does {Customer} have open"
			]
